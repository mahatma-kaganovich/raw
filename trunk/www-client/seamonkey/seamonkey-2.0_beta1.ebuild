...
for l in ${LANGS}; do
	...
	if [[ "${l1}" == "${l2}" ]] ; then
		...
		l2l="${l2l}
		${l1}) echo \"${l2}\" ;;"
		l2="crypt? ( !moznomail? ( http://www.mozilla-enigmail.org/download/release/0.96/enigmail-${l2}-${EMVER}.xpi ) )"
	fi
	SRC_URI="${SRC_URI} linguas_${l1}? ( ${l2} http://releases.mozilla.org/pub/mozilla.org/${PN}/releases/${MY_PV}/langpack/${MY_P}.${l1/_/-}.langpack.xpi )"
done
eval "
l2l(){
case \$1 in
...
src_install() {
	...
	for l in ${LINGUAS}; do
		...
		l="$(l2l $l)"
		[[ -z "${l}" ]] && continue
		xpi_install "${WORKDIR}/enigmail-${l}-${EMVER}"
	done
