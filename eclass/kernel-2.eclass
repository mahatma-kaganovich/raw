...
"
	...
	grep -q "^CONFIG_SQUASHFS=" .config && for i in $COMP; do
		( [[ "$i" == GZIP ]] || grep -q "^CONFIG_SQUASHFS_$i=" .config ) && ( mksquashfs -help-section comp |& grep -q "^\s*${i,,}\s*" ) && comp="${i,,}"
	done
...
userspace(){
	...
	echo '
KLIBCOPTFLAGS += --param=max-grow-copy-bb-insns=1 -fcommon' | tee -a "$sdir"/usr/klibc/arch/*/MCONFIG
