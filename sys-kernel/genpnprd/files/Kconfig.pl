...
$ENV{KERNEL_CONFIG}||='
	...
	-NFS_USE_LEGACY_DNS =NFSD_V.+ -CIFS_.+2
	=(?:JOYSTICK|USB|ISDN|JFFS2|MOUSE|RTC|SQUASHFS|ROMFS|BT|DSCC4|ATM|HISAX|GPIO|A11Y|SPI|BCMA|RT2800USB|HOTPLUG_PCI|CIFS|NFS)_.+
	=MEDIA_.+_SUPPORT =MEDIA_CONTROLLER =VIDEO_.+_RC
	=DRM_GMA.+
	-USB_OTG_(?:WHITELIST|BLACKLIST_HUB)
	=/drivers/(?:mfd|regulator)/.*
	~HISAX_NO_.*
	=.+_(?:PARTITION|FF|X_COMPATIBILITY|PTP|IOMMU|THERMAL|DUAL_ROLE|BY_BOTH|API)
