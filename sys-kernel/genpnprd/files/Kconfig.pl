...
$ENV{KERNEL_CONFIG}||='
	...
	=(?:JOYSTICK|USB|ISDN|JFFS2|MOUSE|RTC|SQUASHFS|ROMFS|BT|DSCC4|ATM|HISAX|GPIO|A11Y|SPI|BCMA|RT2800USB|HOTPLUG_PCI|CIFS|NFS|NFC|OF|POWER_RESET|THERMAL_GOV)_.+
	...
	~HISAX_NO_.+
	=.+_(?:PARTITION|FF|X_COMPATIBILITY|PTP|IOMMU|THERMAL|DUAL_ROLE|BY_BOTH|API|THERMAL)
	...
	IOSF_MBI X86_INTEL_LPSS X86_AMD_PLATFORM_DEVICE
	CPU_SUP_INTEL==!y;X86_INTEL_LPSS CPU_SUP_INTEL==!y;=.+_BAYTRAIL CPU_SUP_INTEL==y;-(?:IOSF_MBI|X86_INTEL_LPSS)
	CPU_SUP_AMD==!y;X86_AMD_PLATFORM_DEVICE CPU_SUP_AMD==y;-X86_AMD_PLATFORM_DEVICE
	=DRM_MEDFIELD =.+_(?:SOC|SCU)(?:_.+)?
	...
	#######
	';
$ENV{KERNEL_CONFIG2}||='?DMA_ENGINE (?:.+_)?PTP_.*CLOCK.*==y;-NETWORK_PHY_TIMESTAMPING';
