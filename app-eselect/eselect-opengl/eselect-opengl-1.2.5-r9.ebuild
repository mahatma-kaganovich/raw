...
# restored for keep some acceleration for rare cards
...
SRC_URI="ftp://ftp.freedesktop.org/pub/mesa/older-versions/7.x/7.11.2/MesaLib-7.11.2.tar.bz2
...
src_unpack(){
	unpack $A
	cp "${WORKDIR}"/Mesa-7.11.2/include/GL/gl*ext.h "${WORKDIR}"/ || cp "${DISTDIR}"/*.h "${WORKDIR}"/ || die
}
